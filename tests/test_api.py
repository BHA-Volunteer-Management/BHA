...
class ApiEndpointsTests(TestCase):
    ...
    def test_referral_successfully_linked(self):
        ...
        response = self.c.get('/api/volunteers/me/')
        self.assertEqual(int(user2.volunteer.referrer), user1.id)

    # many to one integrity
    def test_multiple_referents_single_referrer(self):
        user1 = self.signup('fuzt@buzz.com', 'password')
        user2 = self.signup('reft@ref.com', 'password', referrer=user1.email)
        user3 = self.signup('refz@ref.com', 'password', referrer=user1.email)
        self.assertEqual(int(user2.volunteer.referrer), user1.id)
        self.assertEqual(int(user3.volunteer.referrer), user1.id)

    def test_referral_numbers(self):
        user1 = self.signup('fusz@buzz.com', 'password')
        user2 = self.signup('refff@ref.com', 'password', referrer=user1.email)
        user3 = self.signup('refzf@ref.com', 'password', referrer=user1.email)
        user4 = self.signup('reffft@ref.com', 'password', referrer=user1.email)
        user5 = self.signup('refzft@ref.com', 'password', referrer=user3.email)
        response = self.c.get('/api/volunteers/{}/volunteer_refers/'.format(user1.id))
        self.assertEqual(int(response.content), 3)
        response2 = self.c.get('/api/volunteers/{}/volunteer_refers/'.format(user3.id))
        self.assertEqual(int(response2.content), 1)
        response3 = self.c.get('/api/volunteers/{}/volunteer_refers/'.format(user4.id))
        self.assertEqual(int(response3.content), 0)
